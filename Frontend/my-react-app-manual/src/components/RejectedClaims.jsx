    ...
    <div className="bg-white rounded-lg shadow p-4">
      ...
      <div className="grid grid-cols-2 gap-4 mb-4">
        <div className="bg-gray-50 p-3 rounded-lg text-center">
          <h3 className="text-3xl font-bold text-red-600">{data.rejected}</h3>
          <p className="text-sm text-gray-600">Rejected Claims</p>
          <p className="text-xs text-gray-500 mt-1">~{((data.rejected / data.total_claims) * 100).toFixed(1)}% of total claims</p>
        </div>
        
        <div className="bg-gray-50 p-3 rounded-lg text-center">
          <h3 className="text-3xl font-bold text-orange-600">{data.repudiated}</h3>
          <p className="text-sm text-gray-600">Repudiated Claims</p>
          <p className="text-xs text-gray-500 mt-1">~{((data.repudiated / data.total_claims) * 100).toFixed(1)}% of total claims</p>
        </div>
      </div>
